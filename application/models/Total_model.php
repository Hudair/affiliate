...
class Total_model extends MY_Model{
	...
	public function getUserBalance($user_id, $filter = []){
		...
		$sale_localstore_commission_query = $this->db->query("SELECT `wallet`.`amount` as commission FROM `order_products` INNER JOIN `order` o ON o.id= `order_products`.`order_id` INNER JOIN `wallet` ON `wallet`.`reference_id`= o.id WHERE `wallet`.`user_id` = {$user_id} AND `wallet`.`status` > 0 AND `wallet`.`type` IN ('sale_commission','refer_sale_commission') {$wallet_where} group by `wallet`.`id` ")->result();
