...
<section class="single-product">
  <div class="container">
    ...
    <p><a href="<?= $home_link ?>">Home</a> / <a href="<?= $base_url ?>category"><?= __('store.categories') ?></a> / <?php if($categories){ 
  ...
  <div class="single-product-row">
    <div class="produc-gallery">
      <div id="productSlider" class="carousel slide" data-ride="carousel" data-interval="false">
        ...
        <span><?= $numberOfRatings ?> <?= __('store.customer_review') ?></span>              
			  <span class="spacer">|</span>
        ...
        <span><?= __('store.promoted_by') ?> : 
          ...
          <?php } else { ?>
            <?= (!empty($user['username'])) ? $user['username'] : "admin";?>
          <?php } ?>
        </span>
			  <span class="spacer">|</span>
        <span>Category : <?php 
       ...
       <?php
        ...
        ?>			 
          ...
          <div class="quantity-area" id="field1">
					...
					<button data-product_id="<?= $product['product_id'] ?>" data-product_name="<?= (!empty($product['product_name'])) ? $product['product_name'] : 'Lorem Ipsum' ?>" class="btn btn-cart-detail bg-main2 btn-cart"><?= __('store.add_to_cart') ?></button>
          ...
          <div class="coupon-msg mt-3"></div>
			  ...
			  <div class="variation-row mt-3">
			    <span class="varition-title"><?= __('store.payment') ?>:</span>
				  <div class="variations-payment-row">
            <ul class="pg-listing">
              <?php 
              $payments = get_payment_gateways();
              foreach ($payments as $key => $payment) {
                  if($payment['status'] && $payment['is_install']){
      ...
      <?php if(!empty($ratings)) { ?>
        <?php foreach($ratings as $rating) { ?>
          <div class="reviews-box">
              ...
              <?php } else { ?>
                <img src="<?= base_url('assets/store/default/'); ?>img/blog1.png" alt="<?= __('store.user') ?>" />
              <?php } ?>
            </div>
