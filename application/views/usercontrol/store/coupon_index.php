...
<br>
				...
				<div class="card m-b-30">
					<div class="card-header">
						...
						<div class="pull-right">
							<a class="btn btn-primary" href="<?= base_url('usercontrol/store_coupon_manage/')  ?>"><?= __('admin.add_new'); ?></a>
						</div>
					</div>
									...
									<tbody>
										<?php foreach($coupons as $coupon){ ?>
											<tr>
												...
												<td>
													<a href="<?= base_url('usercontrol/store_coupon_manage/'.$coupon['coupon_id'])  ?>" class="btn btn-primary edit-button" id="<?= $coupon['id'] ?>"><?= __("admin.edit") ?></a>
													<a href="<?= base_url('usercontrol/store_coupon_delete/'.$coupon['coupon_id'])  ?>" class="btn btn-danger delete-button" id="<?= $coupon['id'] ?>"><?= __("admin.delete") ?></a>
												</td>
											</tr>
										<?php } ?>
									</tbody>
	...
	<script type="text/javascript">
		$(".delete-button").on('click',function(){
			return confirm('<?= __('user.are_you_sure') ?>');
		})
	</script>