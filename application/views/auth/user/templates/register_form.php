...
<ul id="registartion_type" class="nav nav-pills mt-4" role="tablist">
	<?php if($registration_status == 1 || $registration_status == 3): ?>
		<li class="nav-item text-center border">
			<a class="nav-link <?= (isset($is_vendor_registration) && $is_vendor_registration) ? "" : "active"; ?>" href="<?php echo base_url('register') ?>" role="tab" data-registartion_type="aff"><?= __('front.affiliate') ?></a>
		</li>
	<?php endif ?>
	
	<?php if($vendor_storestatus && ($registration_status == 1 || $registration_status == 2)): ?>
		<li class="nav-item text-center border">
			<a class="nav-link <?= ((isset($is_vendor_registration) && $is_vendor_registration) || $registration_status == 2) ? "active" : ""; ?>" href="<?php echo base_url('register/vendor') ?>" role="tab" data-registartion_type="ven"><?= __('front.vendor') ?></a>
		</li>
	<?php endif ?>
</ul>
<?php } ?>
<form action="" method="POST" role="form" class="reg_form p-4" novalidate="" autocomple="off" enctype='multipart/form-data'>
	...
	<?php 
		...
		if(isset($read_only_user_membership_plan)){
			...
			$fields['user_membership_plan'] = 
						'<div class="form-group">
							...
							<input readonly type="text"class="form-control custom_input" value="'.$user_membership_plan_value.'">
						</div>';
		}
		...
		if (isset($allow_vendor_option)) {
			...
			$fields['is_vendor'] = '<div class="form-group">
				...
				<select name="is_vendor" class="form-control custom_input">
		...
		$fields['username'] = '<div class="form-group">
			<input type="text" name="username" id="username" class="form-control custom_input" placeholder="'. __('user.username') .'" value="'. $username .'">
		</div>';
		...
		switch ($value['type']) {
			...
			case 'text':
				if($mobile_validation == 'true'){ ?>
					...
					<div class="form-group">
						<div>
							<input id="phone" class="form-control custom_input" type="text" value="<?= $ivalue ?>">
						</div>
					</div>
					<script type="text/javascript">
					...
					if(is_array($ivalue)) {
						...
						<ul class="list-group list-group-flush mb-4">
							...
							<li class="list-group-item d-flex justify-content-between align-items-center">
							    ...
							    <span class="badge badge-danger badge-pill" style="cursor: pointer;" onclick="return this.parentNode.remove();">Delete</span>
	...
	<?php if(!isset($edit_view)){ ?>
		...
		<?php if(isset($template_index)) { ?>
			<div class="check_box text-left"><p>
				<input type="checkbox" id="checkbox" name="terms" value="1" class="" checked>
				<span for="checkbox"><a href="<?= $tnc_link ? $tnc_link : base_url('term-condition') ?>" target="_blank" style="margin-top:0px;"><?= __('user.temrs_and_conditions') ?></a></span>
			</p></div>
		<?php } else { ?>
			<div class="checkbox">
				<label>
				  <input type="checkbox" id="checkbox" name="terms" value="1" class="" checked>
				  <span class="box"></span>
				  <a href="<?= $tnc_link ? $tnc_link : base_url('term-condition') ?>" target="_blank" style="margin-top:0px;"><?= __('user.temrs_and_conditions') ?></a>
				</label>
			</div>
		<?php } ?>

		<input class="<?= $allow_back_to_login ? "btn btn-primary" : ""; ?> btn-registration btn-submit btn continue_btn px-4 text-white round_btn d-block mx-auto" type="submit" value="<?= __('user.submit') ?>">
	<?php } ?>
</form>
...
<script type="text/javascript">
	...
	<?php if(isset($edit_view)){ ?>
		jQuery('.select2').select2({
			placeholder : '<?= (isset($user_groups_readonly) && $user_groups_readonly) ? 'No groups assigned' : 'Assign User Groups'; ?>'
		});
	<?php } ?>
