<?php $k=1; foreach($userslist as $users){  ?>
	...
	<tr>
			...
			<?php
				if($membership['status']){
					...
					if($users['reg_approved'] != 0 && $users['reg_approved'] != 2){ ?>
						<div><a href="javascript:void(0)" edit-plan-user='<?= $users['id'] ?>'><?= __('admin.edit_plan') ?></a></div>
					<?php } else { ?>
						<div>
		...
		<td>
			<?php 
				if (isset($plan) && $plan) { 
					...
					echo '<span class="badge badge-light">'.__('admin.not_available').'</span>';
				}  
			?>
		</td>
		...
		<td>
			...
			<?php if($hasFiles) { ?>
				<i class="fa fa-paperclip mr-2" aria-hidden="true" data-toggle="collapse" data-target="#demo1<?= $k; ?>" class="accordion-toggle"></i>
			<?php } ?>
			...
			<a data-toggle="tooltip" data-original-title="<?= __('admin.edit') ?>" class="btn btn-sm btn-primary" onclick="return confirm('<?= __('admin.are_you_sure_to_edit') ?>');" href="<?php echo base_url();?>admincontrol/addusers/<?php echo $users['id'];?>"><i class="fa fa-edit cursors" aria-hidden="true"></i></a>
			...
			<button data-toggle="tooltip" data-original-title="<?= __('admin.delete') ?>" class="btn btn-sm btn-danger btn-delete2" data-id="<?php echo $users['id'] ?>"><i class="fa fa-trash-o cursors" aria-hidden="true"></i></button>
						...
						<div class='col-sm-4 col-md-3'>
							...
							if( $value['type'] == 'file') {
								...
								if(!isset($valueStored->{'custom_'.$value['name']}) || (empty($valueStored->{'custom_'.$value['name']}) && $valueStored->{'custom_'.$value['name']} != 0)) {
									...
									echo $valueStored->{'custom_'.$value['name']}; 
